...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		leftIcon = new HealthIcon(_song.bf);
		...
		for (awfgaw in 0...Math.round(targetY / 640) + 1920) // grids/steps
		{
			...
			var sectionicon = _song.notes[awfgaw].mustHitSection ? new HealthIcon(_song.bf).clone() : new HealthIcon(_song.enemy).clone();
			sectionicon.x = -95;
	...
	function addSongUI():Void
	{
		...
		{
			_song.bf = characters[Std.parseInt(character)];
		});
		player1DropDown.selectedLabel = _song.bf;

		var player1Label = new FlxText(10,80,64,'Boyfriend');

		var player2DropDown = new FlxUIDropDownMenu(140, 100, FlxUIDropDownMenu.makeStrIdLabelArray(characters, true), function(character:String)
		{
			_song.enemy = characters[Std.parseInt(character)];
		});
		player2DropDown.selectedLabel = _song.enemy;

		var player2Label = new FlxText(140,80,64,'Opponent');

	var player3DropDown = new FlxUIDropDownMenu(player1DropDown.x, player1DropDown.y + 50, FlxUIDropDownMenu.makeStrIdLabelArray(characters, true), function(character:String)
		{
			_song.bf2 = characters[Std.parseInt(character)];
		});
		player3DropDown.selectedLabel = _song.bf2;

		var player3Label = new FlxText(player1Label.x, player1Label.y + 50, 64, 'Boyfriend 2');

		var player4DropDown = new FlxUIDropDownMenu(player2DropDown.x, player2DropDown.y + 50, FlxUIDropDownMenu.makeStrIdLabelArray(characters, true), function(character:String)
		{
			_song.enemy2 = characters[Std.parseInt(character)];
		});
		player4DropDown.selectedLabel = _song.enemy2;

		var player4Label = new FlxText(player2Label.x, player2Label.y + 50, 64, 'Opponent 2');
		...
		if (hasOtherCharacters.checked)
			...
			{
				player3DropDown.visible = false;
				player3Label.visible = false;
				player4DropDown.visible = false;
		...
		*/
		...
		//tab_group_assets.add(hasOtherCharacters);
		...
		check_mustHitSection = new FlxUICheckBox(10, 30, null, null, "Camera Points to P1?", 100,null,function() {
			...
			for (i in sectionRenderes)
			{
				if (i.section.startTime == sect.startTime)
				{
					...
					remove(i.icon);
					var sectionicon = check_mustHitSection.checked ? new HealthIcon(_song.bf).clone() : new HealthIcon(_song.enemy).clone();
					sectionicon.x = -95;
	...
	function addNoteUI():Void
	{
		...
		/*player2 = new Character(0,0, _song.enemy);
		player1 = new Boyfriend(player2.width * 0.2,0 + player2.height, _song.bf);
	...
	override function update(elapsed:Float)
	{
		...
		var numOne = FlxG.keys.justPressed.NUMPADONE; 
		...
		{
			case 0:
				noteTypeText = '1st Player Sings';
			case 1:
				noteTypeText = '2nd Player Sings';
			case 2:
				noteTypeText = 'Duet';
			case 3:
		...
		if (FlxG.keys.justPressed.C && !FlxG.keys.pressed.CONTROL)
		{
			...
			var cachedY = i.icon.y;
			remove(i.icon);
			var sectionicon = sect.mustHitSection ? new HealthIcon(_song.bf).clone() : new HealthIcon(_song.enemy).clone();
			sectionicon.x = -95;
			sectionicon.y = cachedY;
			sectionicon.setGraphicSize(0, 45);
