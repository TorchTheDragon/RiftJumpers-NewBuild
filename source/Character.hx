...
class Character extends FlxSprite
{
	...
	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false)
	{
		...
		switch (curCharacter)
		{
			case 'torch':
				noteSkin = 'torch';
				healthColor = 0xFF9B0000;
				tex = Paths.getSparrowAtlas('torchbf', 'riftjumpers', true);
				frames = tex;
				...
				loadOffsetFile(curCharacter, 'riftjumpers');

				playAnim('idle');
				
				flipX = true; //This might be breaking the Torch character for a background enemy, if so, then I need two make two characters, one for Torch as enemy, and one for Torch as BF
			...
			case 'spooky':
				...
				playAnim('danceRight');
			case 'mom':
				noteSkin = 'torch'; //this is for testing only
			...
			case 'bf-christmas':
				noteSkin = 'normal';
				healthColor = 0xFF31B0D1;
				tex = Paths.getSparrowAtlas('bfChristmas','shared',true);
				frames = tex;
				animation.addByPrefix('idle', 'BF idle dance', 24, false);
				animation.addByPrefix('singUP', 'BF NOTE UP0', 24, false);
				animation.addByPrefix('singLEFT', 'BF NOTE LEFT0', 24, false);
			...
			case 'bf-car':
				noteSkin = 'normal';
				healthColor = 0xFF31B0D1;
				tex = Paths.getSparrowAtlas('bfCar','shared',true);
				frames = tex;
				animation.addByPrefix('idle', 'BF idle dance', 24, false);
				animation.addByPrefix('singUP', 'BF NOTE UP0', 24, false);
				animation.addByPrefix('singLEFT', 'BF NOTE LEFT0', 24, false);
